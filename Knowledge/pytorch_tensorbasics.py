...
# Fancy Indexing
x = torch.arange(10)  # x = [0, 1, 2, ..., 9]
indices = [2, 5, 8]   # Get the 3rd, 6th and 9th element
print(x[indices])  # x[indices] = [2, 5, 8]

x = torch.rand((3, 5)) # Shape 3x5
rows = torch.tensor([1, 0])  # Second row and first row
cols = torch.tensor([4, 0])  # Fifth column and first column
print(x[rows, cols])  # Gets second row fifth column and first row first column  # Shape: [2]
